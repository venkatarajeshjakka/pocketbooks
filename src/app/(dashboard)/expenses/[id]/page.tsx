...
export async function generateMetadata({ params }: ExpenseDetailPageProps) {
    const { id } = await params;
    ...
    try {
        ...
        if (expense) {
            return {
                title: `${expense.description} - ₹${expense.amount.toLocaleString('en-IN')} | PocketBooks`,
                description: `View expense details for ${expense.description}`,
            };
        }
    } catch (error) {
        console.error('Failed to fetch expense for metadata:', error);
    }
